...
export default function OracleExperience(){
 ...
 result?.kind==='tarot'?<div className="tarot-screen"><div className="result-kicker">{result.free?'无牌阵 · 观察三张牌的关系':result.cards.length===3?'三张牌 · 现状 / 阻碍 / 建议':'单张牌 · 当下提示'}</div><div className={'tarot-cards '+(result.cards.length===1?'single':'')}>{result.cards.map(c=><div className={'tarot-card '+(c.isReversed?'tarot-reversed':'tarot-upright')} key={c.id}><small>{c.position}</small><span className={c.isReversed?'reversed':''}><TarotSymbol id={c.id}/></span><b>{c.name}</b><small>{c.isReversed?'逆位':'正位'}</small></div>)}</div><div className="screen-footer">牌影之间，藏着未言的回响。</div></div>:
