...
export default function TarotSummary({cards,free=false,question=''}:{cards:DrawnCard[];free?:boolean;question?:string}) {
 ...
 return <section className="reflection tarot-insight" aria-label="综合分析与建议">
  ...
  <Select value={topic} onValueChange={value=>{if(value&&value in readingTopics)setChosen(value as ReadingTopic)}}>
   <SelectTrigger aria-labelledby="tarot-topic-label"><SelectValue/></SelectTrigger>
  ...
  {reading.insights.map((entry,i)=><div className="tarot-card-insight" key={cards[i].id}><h4>{entry.title}</h4><p>{entry.text}</p><p>{entry.context}</p><p className="method-copy">{entry.check}</p></div>)}
  ...
  <p className="method-copy">{reading.boundary}</p>
 </section>;
}
